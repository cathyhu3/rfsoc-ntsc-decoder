...
async def test_a(dut):
    ...
    outm = AXISMonitor(dut,'m00',dut.s00_axis_aclk, callback = lambda x: received_data.append(x.integer))
    ...
    imag_array_white = np.load(proj_path / "sim" / "imag_array_white.npy")[0:800]
    real_array_white = np.load(proj_path / "sim" / "real_array_white.npy")[0:800]
    t_array_white = np.load(proj_path / "sim" / "t_array_white.npy")[0:800]
    ...
    data = {'type':'write_burst', "contents": {"data": mag_in}}
    ind.append(data)
    pause = {"type": "pause","duration": 1}
    ind.append(pause)

    # Read the processed data.
    data = {'type':'read_burst', "duration": 50}
    outd.append(data)
    outd.append(pause)

    await ClockCycles(dut.s00_axis_aclk, len(mag_in)+100)
...
def hsync_runner(module_name):
    ...
    sources = [proj_path / "hdl" / "hsync_detector.sv"]
...
if __name__ == "__main__":
    hsync_runner("hsync_detector.sv")