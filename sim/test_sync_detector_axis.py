...
async def collect_vals(dut, hsyncs, colorbursts, h_pulses, cb_pulses, odd_even):
    # hsyncs = []
    # colorbursts = []
    # h_pulses = []
    # cb_pulses = []
    while len(cb_pulses) < len(mag_in):
        ...
        if (dut.m00_axis_tvalid.value):
            ...
            odd_even.append(dut.odd_even.value)
...
async def test_a(dut):
    ...
    cocotb.start_soon(collect_vals(dut, hsyncs, colorbursts, h_pulses, cb_pulses, odd_even))
    ...
    for i, b in enumerate(odd_even):
        if b == 1:
            plt.axvline(x=14000+i, linestyle='-', linewidth=2, color="pink", alpha=0.1)
    plt.show()
