...
async def collect_vals(dut, hsyncs, colorbursts):
    hsyncs = []
    colorbursts = []
    while len(hsyncs) < len(mag_in):
        ...
        if (dut.valid_out.value):
            hsyncs.append(dut.hsync.value >> 17)
            colorbursts.append((dut.colorburst.value >> 16) & 1)
...
async def test_a(dut):
    ...
    cocotb.start_soon(Clock(dut.clk, 126, units="ns").start()) # 8 MHz clock
    cocotb.start_soon(collect_vals(dut, hsyncs, colorbursts))
    await reset(dut.clk, dut.resetn,2,0)
